...
Test.define(
  ...
  () => {
    const input = 3.14159;
    Test.assert(numberWithDecimalPlaceLimit(input, 2) === 3.14, "Result should be correct");
  }
);
...
Test.define(
  ...
  () => {
    const input = 3;
    Test.assert(numberWithDecimalPlaceLimit(input, 2) === 3, "Result should be correct");
  }
);
